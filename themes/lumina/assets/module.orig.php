...
function lumina_tpl_url($path = '') {
    if (defined('TEMPLATE_URL')) {
        ...
        $base = rtrim(BLOG_URL, '/') . '/content/templates/' . Option::get('nonce_templet') . '/';
    }
...
function lumina_asset_url($path) {
    ...
    if (defined('TEMPLATE_PATH')) {
        $file = TEMPLATE_PATH . str_replace('/', DIRECTORY_SEPARATOR, $path);
        if (is_file($file)) {
...
function lumina_user_url($path = '', $query = []) {
    $url = rtrim(BLOG_URL, '/') . '/index.php/user';
...
function lumina_can_view_private_log($author) {
    return defined('ISLOGIN') && ISLOGIN && defined('UID') && (int)UID === (int)$author;
}

function lumina_can_manage_log_item($log) {
    if (!defined('ISLOGIN') || !ISLOGIN || !is_array($log)) {
        return false;
    }
    $author = isset($log['author']) ? (int)$log['author'] : 0;
    return (defined('UID') && (int)UID === $author) || (class_exists('User') && User::haveEditPermission());
}
...
function lumina_safe_custom_url($url) {
    ...
    return defined('BLOG_URL') ? rtrim(BLOG_URL, '/') . '/' . ltrim($url, '/') : $url;
}
...
function lumina_get_quick_nav_items($fallbackSorts = []) {
    ...
    $items = [
        [
            'label' => '全部',
            'url' => htmlspecialchars(defined('BLOG_URL') ? BLOG_URL : '/', ENT_QUOTES),
        ],
    ];
...
function lumina_get_custom_float_buttons() {
    ...
    if (lumina_truthy_option(lumina_opt('custom_float_enable', 'n'))) {
        ...
        $vam_items = function_exists('_vam') ? _vam('custom_float_items', null) : null;
...
function lumina_get_friend_links() {
    ...
    if (isset($CACHE) && is_object($CACHE) && method_exists($CACHE, 'readCache')) {
        ...
        $cache = method_exists('Cache', 'getInstance') ? Cache::getInstance() : new Cache();
...
function lumina_resolve_url($url, $fallback = '') {
    ...
    if (strpos($url, 'images/') === 0) {
        return TEMPLATE_URL . 'assets/img/' . substr($url, strlen('images/'));
    }
    ...
    if (defined('TEMPLATE_PATH') && file_exists(TEMPLATE_PATH . 'assets/img/' . $url)) {
        return TEMPLATE_URL . 'assets/img/' . $url;
    }
...
function lumina_render_emoji_panel() {
    $emojiDir = defined('TEMPLATE_PATH') ? TEMPLATE_PATH . 'assets/owo/paopao/' : '';
    ...
    foreach ($emojiList as $emojiFile) {
        ...
        $alt = '::(' . $alt . ')';
        $emojiUrl = TEMPLATE_URL . 'assets/owo/paopao/' . $emojiName;
...
if (!function_exists('lumina_prepare_redpacket_state')) {
    function lumina_prepare_redpacket_state($gid, $fields = []) {
        ...
        if (!empty($redpacket)) {
            ...
            if (defined('ISLOGIN') && ISLOGIN && defined('UID')) {
                $claim = lumina_redpacket_get_claim((int)$redpacket['id'], UID);
...
if (!function_exists('lumina_prepare_link_card_state')) {
    function lumina_prepare_link_card_state($fields = []) {
        ...
        } elseif (strpos($hostUrl, '/') === 0 && defined('BLOG_URL')) {
            $hostUrl = rtrim(BLOG_URL, '/') . $hostUrl;
        }
        $host = parse_url($hostUrl, PHP_URL_HOST);
        if (!$host && defined('BLOG_URL')) {
    ...
    function lumina_render_link_card($state, $compact = false, $wholeLink = true, $showAction = true) {
        ...
        <<?= $wholeLink ? 'a' : 'span' ?> class="<?= $class ?>"<?= $wholeLink ? ' href="' . htmlspecialchars($url, ENT_QUOTES) . '" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation();"' : '' ?>>
            ...
            <span class="lumina-link-card-thumb">
                <?php if ($image !== ''): ?>
                    <img src="<?= TEMPLATE_URL ?>assets/img/thumbnail.svg" data-src="<?= htmlspecialchars($image, ENT_QUOTES) ?>" alt="">
                <?php else: ?>
    ...
    function lumina_render_main_footer() {
        ...
        if ($lumina_footer_info === '') {
            ...
            if ($siteName === '' && class_exists('Option')) {
                $siteName = trim((string)Option::get('blogname'));
            }
            if ($siteName !== '') {
        ...
        if ($icp_value === '' && class_exists('Option')) {
            $icp_value = (string)Option::get('icp');
        }
        ?>
    <div class="sh-footer">
...
if (!function_exists('lumina_render_redpacket_card')) {
    function lumina_render_redpacket_card($gid, $authorName, $state) {
        ...
        ob_start();
        ?>
        <div class="lumina-redpacket-card<?= $disabled ?>" data-gid="<?= (int)$gid ?>" data-login="<?= (defined('ISLOGIN') && ISLOGIN) ? '1' : '0' ?>" data-claimed="<?= $claimed ? '1' : '0' ?>" data-status="<?= $status === 1 ? '1' : '0' ?>" data-token="<?= (defined('ISLOGIN') && ISLOGIN) ? LoginAuth::genToken() : '' ?>" data-blog-url="<?= BLOG_URL ?>" data-claim-url="<?= htmlspecialchars(function_exists('lumina_user_url') ? lumina_user_url() : (rtrim(BLOG_URL, '/') . '/index.php/user'), ENT_QUOTES) ?>">
    ...
    function lumina_render_sidebar_profile_card($profileUid, $profileAvatar, $fallbackAvatar, $profileName, $sidebarContact, $sidebarStats) {
        ...
        <div class="lumina-sidecard lumina-sidecard-profile">
            <div class="lumina-sidecard-profile-head">
                <a class="lumina-sidecard-avatar" href="<?= Url::author($profileUid) ?>">
...
function lumina_get_user_avatar($uid, $fallback = '') {
    ...
    if (!empty($user['photo'])) {
        if (class_exists('User') && method_exists('User', 'getAvatar')) {
            return lumina_resolve_avatar(User::getAvatar($user['photo']), $fallback);
        }
...
function getEmUserAvatar($uid, $mail) {
    $uid = (int)$uid;
    if ($uid > 0) {
        return lumina_get_user_avatar($uid, lumina_opt('default_avatar', TEMPLATE_URL . 'assets/img/tx.png'));
    }
    if (!empty($mail) && function_exists('getGravatar')) {
        $avatar = getGravatar($mail);
        if (!empty($avatar)) {
            return lumina_resolve_avatar($avatar, lumina_opt('default_avatar', TEMPLATE_URL . 'assets/img/tx.png'));
        }
    }
    return lumina_resolve_avatar(BLOG_URL . 'admin/views/images/avatar.svg', lumina_opt('default_avatar', TEMPLATE_URL . 'assets/img/tx.png'));
}
...
function lumina_resolve_media_url($file_path) {
    ...
    if (stripos($file_path, 'content/uploadfile/') !== false) {
        ...
        return BLOG_URL . substr($file_path, $pos);
    }
...
if (!function_exists('blog_sort')) {
    function blog_sort($sortid) {
        ...
        $url = htmlspecialchars((string)Url::sort($sortid), ENT_QUOTES, 'UTF-8');
...
if (!function_exists('blog_comments')) {
    function blog_comments($data, $comnum = 0, $logid = null) {
        ...
        $fallback_avatar = lumina_opt('default_avatar', TEMPLATE_URL . 'assets/img/tx.png');
        if ($fallback_avatar === '') {
            $fallback_avatar = TEMPLATE_URL . 'assets/img/tx.png';
        }
        ...
        foreach ($stack as $cid) {
            ...
            if ($uid > 0) {
                $posterHtml = '<a href="' . htmlspecialchars(Url::author($uid)) . '">' . $poster . '</a>';
            } elseif ($url !== '') {
...
if (!function_exists('blog_comments_post')) {
    function blog_comments_post($logid, $ckname = '', $ckmail = '', $ckurl = '', $verifyCode = '', $allow_remark = 'y') {
        ...
        if (User::isVisitor() && $isLoginComment === 'y') {
        ...
        echo '<form class="sh-pinglun" method="post" action="' . BLOG_URL . '?action=addcom">';
        echo '<input type="hidden" name="gid" value="' . $logid . '">';
        echo '<input type="hidden" name="pid" value="0">';
        if (User::isVisitor() && $isLoginComment === 'n') {
